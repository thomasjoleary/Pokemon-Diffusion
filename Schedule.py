...
def forward_noise(image, step, alphas):
    ...
    noisy = torch.sqrt(alphas[step].view(-1, 1, 1, 1)) * image + torch.sqrt(1 - alphas[step].view(-1, 1, 1, 1)) * noise
    return noisy, noise
...
if __name__ == "__main__":
    ...
    img, _ = PokemonDataset.__getitem__(dataset, 0)
    img = img.unsqueeze(0)
    ...
    for step in range(len(showSteps)):
        noisy, noise = forward_noise(img, torch.tensor([showSteps[step]]), alphas)
